...
从因子分解的角度看，马尔科夫网存在着不好的模糊性。考虑图2.2（左）的3变量马尔科夫网。任何按照$$p(y_1,y_2,y_3)\varpropto f(y_1,y_2，y_3)$$分解的分布，都可能与它对应。然而，我们希望使用更严格的参数化——$$p(y_1,y_2,y_3)=f(y_1,y_2)g(y_2,y_3)h(y_1,y_3)$$。后面这组模型簇是前面的严格子集，且需要更少的数据来获得准确的分布估计（译注：参数估计？）。然而，马尔科夫网不能区分这两种参数化。相反，因子图无歧义地描述了模型的因子分解。

![](/assets/QQ截图20171230123923.png)
图2.2 带有模糊性的马尔科夫网（左）。右边的两种分解都有可能与左图对应。
